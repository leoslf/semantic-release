...
import { RELEASE_TYPE, DEFAULT_FIRST_RELEASE } from "../definitions/constants.js";
import {
  getFirstVersion,
  getLatestVersion,
  getLowerBound,
...
export function maintenance({ maintenance, release }, { firstRelease = DEFAULT_FIRST_RELEASE } = {}) {
  return sortBy(
    ...
    const min = highest(getLatestVersion(versions) || firstRelease, maintenanceMin);
...
export function release({ release }, { firstRelease = DEFAULT_FIRST_RELEASE } = {}) {
  if (release.length === 0) {
    return release;
  }

  // The initial lastVersion is the last release from the base branch of `firstRelease` (1.0.0)
  let lastVersion = getLatestVersion(tagsToVersions(release[0].tags)) || firstRelease;
