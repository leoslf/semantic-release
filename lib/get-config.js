...
export default async (context, cliOptions) => {
  ...
  return {
    ...
    packageVersion: (await pkgVersion({ normalize: false, cwd })) || (await pkgVersion({ cwd, env })),
  };
};

async function pkgVersion(options) {
  const { packageJson: { version } = {} } = (await readPackageUp(options)) || {};
  return version;
}
